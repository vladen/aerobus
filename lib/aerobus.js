...
(function (global, factory) {
      ...
      value: function publish(data, callback) {
        ...
        if (isSomething(callback)) {
          (function () {
            ...
            var results = [];
            getGear(_this).publish(data, function (result) {
        ...
        if (retentions) for (var i = -1, l = subscribers.length; ++i < l;) {
          ...
          var _loop3 = function _loop3(j, m) {
            ...
            try {
              ...
              setImmediate(function () {
                return _this5.bus.error(error, retention);
              });
            }
          };
    ...
    function ChannelBase(bus, name, parent) {
      ...
      if (patterns.length) {
        ...
        for (var i = -1, l = patterns.length; ++i < l;) {
          ...
          if (pattern.regex.test(name)) pattern.replay(replayee);
        }
      }
  ...
  var Pattern = (function (_Replay) {
    _inherits(Pattern, _Replay);

    function Pattern(regex) {
      _classCallCheck(this, Pattern);

      var _this10 = _possibleConstructorReturn(this, Object.getPrototypeOf(Pattern).call(this));
  ...
  var SectionBase = (function (_Common2) {
    ...
    function SectionBase(bus, resolver) {
      ...
      var _this11 = _possibleConstructorReturn(this, Object.getPrototypeOf(SectionBase).call(this));

      setGear(_this11, new SectionGear(bus, resolver));
      return _this11;
    }
  ...
  var WhenGear = (function (_Replay2) {
    _inherits(WhenGear, _Replay2);

    function WhenGear(bus, parameters, targets) {
      _classCallCheck(this, WhenGear);

      var _this13 = _possibleConstructorReturn(this, Object.getPrototypeOf(WhenGear).call(this));

      _this13.condition = truthy;
      _this13.sources = [];
      _this13.targets = targets;
      ...
      for (var i = -1, l = parameters.length; ++i < l;) {
        ...
        switch (classOf(_parameter)) {
          case CLASS_FUNCTION:
            _this13.condition = _parameter;
            break;

          case CLASS_STRING:
            _this13.sources.push(_parameter);
      ...
      switch (_this13.sources.length) {
        ...
        case 1:
          _this13.observer = {
            ...
            next: function next(message) {
              if (_this13.condition(message)) _this13.replay(_this13.targets);
            }
          };
          (_this13.sources[0] = getGear(bus.get(_this13.sources[0]))).observe(_this13.observer);
          break;

        default:
          _this13.counters = new Map();
          _this13.observer = {
            done: noop,
            next: function next(message) {
              if (!_this13.condition(message)) return;
              var destination = message.destination,
                  counter = _this13.counters.get(destination) + 1;

              _this13.counters.set(destination, counter);

              for (var i = -1, l = _this13.counters.length; ++i < l;) {
                if (_this13.counters[i] !== counter) return;
              }

              _this13.replay(_this13.targets);
            }
          };

          for (var i = _this13.sources.length - 1; i >= 0; i--) {
            (_this13.sources[i] = getGear(bus.get(_this13.sources[i]))).observe(_this13.observer);
          }
    ...
    _createClass(BusGear, [{
      ...
      value: function resolve(names) {
        ...
        for (var i = -1, l = names.length; ++i < l;) {
          ...
          switch (classOf(name)) {
            case CLASS_REGEXP:
              if (!(name in patterns)) patterns.push(patterns[name] = new Pattern(name));
              break;
        ...
        return new Section(this, function () {
          ...
          for (var i = -1, l = names.length; ++i < l;) {
            var name = names[i];
            if (isString(name)) channels.push(_this16.get(name));else {
              var _iteratorNormalCompletion3 = true;
              var _didIteratorError3 = false;
              var _iteratorError3 = undefined;

              try {
                for (var _iterator3 = _this16.channels.values()[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
                  var channel = _step3.value;
                  if (name.test(channel.name)) channels.push(channel);
                }
              } catch (err) {
