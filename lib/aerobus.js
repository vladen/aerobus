...
(function (global, factory) {
  ...
  var IteratorGear = (function () {
    function IteratorGear(observables) {
      ...
      var count = 0,
          ...
          _this4.rejects.forEach(function (reject) {
            return reject();
          });
        },
        next: function next(message) {
          if (_this4.resolves.length) _this4.resolves.shift()(message);else _this4.messages.push(message);
        }
      };
      ...
      value: function next() {
        ...
        return {
          value: new Promise(function (resolve, reject) {
            _this5.rejects.push(reject);

            _this5.resolves.push(resolve);
          })
        };
      }
    }]);
  ...
  var WhenGear = (function (_Replay) {
    ...
    function WhenGear(bus, parameters, targets) {
      ...
      var _this6 = _possibleConstructorReturn(this, Object.getPrototypeOf(WhenGear).call(this));

      _this6.condition = truthy;
      _this6.sources = [];
      _this6.targets = targets;
      ...
      for (var i = -1, l = parameters.length; ++i < l;) {
        ...
        switch (classOf(_parameter)) {
          case CLASS_FUNCTION:
            _this6.condition = _parameter;
            break;

          case CLASS_STRING:
            _this6.sources.push(_parameter);
      ...
      switch (_this6.sources.length) {
        ...
        case 1:
          _this6.observer = {
            ...
            next: function next(message) {
              if (_this6.condition(message)) _this6.replay(_this6.targets);
            }
          };
          (_this6.sources[0] = getGear(bus.get(_this6.sources[0]))).observe(_this6.observer);
          break;

        default:
          _this6.counters = new Map();
          _this6.observer = {
            done: noop,
            next: function next(message) {
              if (!_this6.condition(message)) return;
              var destination = message.destination,
                  counter = _this6.counters.get(destination) + 1;

              _this6.counters.set(destination, counter);

              for (var i = -1, l = _this6.counters.length; ++i < l;) {
                if (_this6.counters[i] !== counter) return;
              }

              _this6.replay(_this6.targets);
            }
          };

          for (var i = _this6.sources.length - 1; i >= 0; i--) {
            (_this6.sources[i] = getGear(bus.get(_this6.sources[i]))).observe(_this6.observer);
          }
      ...
      value: function publish(data, callback) {
        ...
        if (isSomething(callback)) {
          (function () {
            ...
            var results = [];
            getGear(_this7).publish(data, function (result) {
  ...
  var ChannelBase = (function (_Common) {
    ...
    function ChannelBase(bus, name, parent) {
      ...
      var _this8 = _possibleConstructorReturn(this, Object.getPrototypeOf(ChannelBase).call(this));

      objectDefineProperty(_this8, 'name', {
        value: name,
        enumerable: true
      });
      if (isSomething(parent)) objectDefineProperty(_this8, 'parent', {
        value: parent,
        enumerable: true
      });
      ...
      setGear(_this8, new ChannelGear(bus, name, parent, trace));
      return _this8;
    }
