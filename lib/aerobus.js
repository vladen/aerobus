  ...
  var IteratorGear = (function () {
    function IteratorGear(channels) {
      ...
      this.disposer = function () {
        for (var i = channels.length - 1; i >= 0; i--) {
          var channel = channels[i],
              ...
              index = iterators ? iterators.indexOf(iterator) : -1;
          if (! ~index) return;
      ...
      for (var i = channels.length - 1; i >= 0; i--) {
        var channel = channels[i],
            iterators = channel.iterators;
        if (iterators) iterators.push(iterator);else channel.iterators = [iterator];
      }
    }
  ...
  var ChannelGear = (function () {
    ...
    _createClass(ChannelGear, [{
      ...
      value: function clear() {
        ...
        if (iterators) {
          for (var i = iterators.length - 1; i >= 0; i--) {
            iterators[i].done();
          }
      ...
      value: function publish(message, respond) {
        ...
        if (iterators) for (var i = 0, l = iterators.length; i < l; i++) {
          iterators[i].next(message);
        }
      ...
      value: function reset() {
        ...
        if (iterators) for (var i = iterators.length - 1; i >= 0; i--) {
          iterators[i].done();
        }
        if (subscribers) for (var i = subscribers.length - 1; i >= 0; i--) {
          try {
            subscribers[i].done();
          } catch (error) {
            bus.error(error);
      ...
      value: function shuffle(limit) {
        ...
        if (limit) this.strategy = function (subscriptions) {
          ...
          do {
            var candidate = subscriptions[mathFloor(mathRandom() * length)];
      ...
      value: function subscribe(subscription) {
        ...
        var bus = this.bus,
            ...
            retentions = this.retentions;
        if (collection) for (var i = 0, l = subscribers.length; i < l; i++) {
          var subscriber = subscribers[i],
          ...
          if (retentions) for (var j = 0, m = retentions.length; j < m; j++) {
        ...
        if (retentions) for (var i = 0, l = subscribers.length; i < l; i++) {
          var subscriber = subscribers[i];

          for (var j = 0, m = retentions.length; j < m; j++) {
      ...
      value: function unsubscribe(parameters) {
        ...
        if (parameters.length) {
          var _unsubscribe = function _unsubscribe(predicate) {
          ...
          var _loop = function _loop(i, l) {
            ...
            switch (classof(parameter)) {
              case CLASS_AEROBUS_SUBSCRIBER:
                _unsubscribe(function (subscriber) {
  ...
  var ChannelBase = (function () {
    function ChannelBase(bus, name, parent) {
      ...
      var trace = function trace(event) {
        ...
        return bus.trace.apply(bus, [event, _this4].concat(args));
      };
      ...
      value: function publish(data, callback) {
        ...
        if (isSomething(callback)) {
          (function () {
            ...
            var results = [];
            getGear(_this5).publish(data, function (result) {
      ...
      value: function publish(data, callback) {
        ...
        if (isSomething(callback)) {
          (function () {
            if (!isFunction(callback)) throw errorCallbackNotValid(callback);
            var results = [];
            getGear(_this8).apply('publish', data, function (result) {
              return results.push(result);
            });
            callback(results);
          })();
