  ...
  var Subscriber = function Subscriber(base, name, order) {
    ...
    if (isFunction(base)) {
      ...
      if (isSomething(base.done)) {
        if (isFunction(base.done)) done = function () {
          return base.done();
  ...
  var IteratorGear = (function () {
    function IteratorGear(channels) {
      ...
      this.disposer = function () {
        for (var i = channels.length; i--;) {
          ...
          if (collection) for (var j = collection.length; --j;) {
            ...
            if (item === observer) collection[j] = null;else observers.push(item);
          }
          if (observers.length) channel.observers = observers;else delete channel.observers;
        }
      };
      ...
      for (var i = channels.length; i--;) {
        ...
        if (observers) channel.observers = observers.concat(observer);else channel.observers = [observer];
      }
    }
        ...
        if (retentions) for (var i = -1, l = subscribers.length; ++i < l;) {
          ...
          var _loop2 = function _loop2(j, m) {
            ...
            try {
              ...
              setImmediate(function () {
                return _this7.bus.error(error, retention);
              });
            }
          };
      ...
      value: function publish(data, callback) {
        ...
        if (isSomething(callback)) {
          (function () {
            ...
            var results = [];
            getGear(_this10).publish(data, function (result) {
      ...
      value: function publish(data, callback) {
        ...
        if (isSomething(callback)) {
          (function () {
            if (!isFunction(callback)) throw errorCallbackNotValid(callback);
            var results = [];
            getGear(_this13).apply('publish', data, function (result) {
              return results.push(result);
            });
            callback(results);
          })();
