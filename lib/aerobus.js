...
(function (global, factory) {
  ...
  var CLASS_AEROBUS = 'Aerobus',
    ...
    return target;
      ...
      finalize = function finalize(collection, error) {
    ...
    return new TypeError('Delimiter expected to be not empty string, not "' + value + '".');
      ...
      errorDependencyNotValid = function errorDependencyNotValid() {
    ...
    return new TypeError('Order expected to be a number, not "' + classOf(value) + '".');
      ...
      errorReferenceNotValid = function errorReferenceNotValid(value) {
    return new TypeError('Reference expected to be a regexp or a string, not "' + classOf(value) + '".');
  },
    ...
    return new TypeError('Trace expected to be a function, not "' + classOf(value) + '".');
      ...
      errorWhenExtensionNotValid = function errorWhenExtensionNotValid(value) {
  ...
  var Resolver = (function () {
    function Resolver(bus, parameters) {
      var _this = this;
      ...
      var _loop = function _loop(i, l) {
        ...
        switch (classOf(parameter)) {
          case CLASS_BOOLEAN:
            _this.dynamic = parameter;
            break;

          case CLASS_REGEXP:
            resolvers.push(function (resolved) {
              var channels = Array.from(bus.channels.values());

              for (var j = -1, m = channels.length; ++j < m;) {
                var channel = channels[j];
                if (parameter.test(channel.name)) resolved.add(channel);
              }
            });
            var predicate = _this.predicate;
            if (predicate) _this.predicate = function (channel) {
              return parameter.test(channel.name) && predicate(channel);
            };else _this.predicate = function (channel) {
              return parameter.test(channel.name);
            };
            break;

          case CLASS_STRING:
            resolvers.push(function (resolved) {
              return resolved.add(bus.get(parameter));
            });
            break;
      ...
      if (this.dynamic) this.recording = new Recording();
    }

    _createClass(Resolver, [{
      key: 'notify',
      value: function notify(channel) {
        if (!this.predicate || !this.predicate(channel)) return;
        var recording = this.recording;
        if (recording) recording.replay(channel);
      }
    }, {
      key: 'record',
      ...
      value: function resolve() {
        ...
        for (var i = -1, l = resolvers.length; ++i < l;) {
          resolvers[i](resolved);
        }

        resolved = Array.from(resolved.values());
        if (!this.dynamic) this.resolved = resolved;
        return resolved;
      }
    }]);
  ...
  var BusGear = (function () {
    ...
    _createClass(BusGear, [{
      ...
      key: 'create',
      value: function create(name, parent) {
      ...
      key: 'resolve',
      value: function resolve(parameters) {
        ...
        if (resolver.dynamic) this.resolvers.push(resolver);
    ...
    _createClass(ChannelGear, [{
      ...
      value: function publish(message, callback) {
        ...
        for (var i = -1, l = subscribers.length; ++i < l;) {
          ...
          if (subscriber) try {
            ...
            callback(error);
            setImmediate(function () {
              return _this2.bus.error(error, message);
            });
          }
        }
      }
      ...
      value: function subscribe(subscription) {
        ...
        if (retentions) for (var i = -1, l = subscribers.length; ++i < l;) {
          ...
          var _loop2 = function _loop2(j, m) {
            ...
            try {
              ...
              setImmediate(function () {
                return _this3.bus.error(error, retention);
              });
            }
          };
    ...
    for (var i = -1, l = parameters.length; ++i < l;) {
      ...
      switch (classOf(_parameter)) {
        case CLASS_AEROBUS_FORWARDING:
          forwarders.push.apply(forwarders, _toConsumableArray(_parameter.forwarders));
          break;

        case CLASS_FUNCTION:
        case CLASS_STRING:
          forwarders.push(_parameter);
          break;
  ...
  var WhenGear = (function (_Recording) {
    _inherits(WhenGear, _Recording);

    function WhenGear(bus, parameters, targets) {
      _classCallCheck(this, WhenGear);

      var _this7 = _possibleConstructorReturn(this, Object.getPrototypeOf(WhenGear).call(this));

      _this7.condition = truthy;
      _this7.sources = [];
      _this7.targets = targets;

      for (var i = -1, l = parameters.length; ++i < l;) {
        var _parameter2 = parameters[i];

        switch (classOf(_parameter2)) {
          case CLASS_FUNCTION:
            _this7.condition = _parameter2;
            break;

          case CLASS_STRING:
            _this7.sources.push(_parameter2);

            break;

          default:
            throw errorArgumentNotValid(_parameter2);
        }
      }

      switch (_this7.sources.length) {
        case 0:
        ...
        case 1:
          _this7.observer = {
            ...
            next: function next(message) {
              if (_this7.condition(message)) _this7.replay(_this7.targets);
            }
          };
          (_this7.sources[0] = getGear(bus.get(_this7.sources[0]))).observe(_this7.observer);
          break;

        default:
          _this7.counters = new Map();
          _this7.observer = {
            done: noop,
            next: function next(message) {
              if (!_this7.condition(message)) return;
              var destination = message.destination,
                  counter = _this7.counters.get(destination) + 1;

              _this7.counters.set(destination, counter);

              for (var i = -1, l = _this7.counters.length; ++i < l;) {
                if (_this7.counters[i] !== counter) return;
              }

              _this7.replay(_this7.targets);
            }
          };

          for (var i = _this7.sources.length - 1; i >= 0; i--) {
            (_this7.sources[i] = getGear(bus.get(_this7.sources[i]))).observe(_this7.observer);
          }
      ...
      value: function publish(data, callback) {
        ...
        if (isSomething(callback)) {
          (function () {
            ...
            var results = [];
            getGear(_this8).publish(data, function (result) {
  ...
  var ChannelBase = (function (_Common) {
    ...
    function ChannelBase(bus, name, parent) {
      ...
      var _this9 = _possibleConstructorReturn(this, Object.getPrototypeOf(ChannelBase).call(this));

      objectDefineProperty(_this9, 'name', {
        value: name,
        enumerable: true
      });
      if (isSomething(parent)) objectDefineProperty(_this9, 'parent', {
        value: parent,
        enumerable: true
      });
      ...
      setGear(_this9, new ChannelGear(bus, name, parent, trace));
      return _this9;
    }
  ...
  var SectionBase = (function (_Common2) {
    ...
    function SectionBase(bus, resolver) {
      ...
      var _this12 = _possibleConstructorReturn(this, Object.getPrototypeOf(SectionBase).call(this));

      setGear(_this12, new SectionGear(bus, resolver));
      return _this12;
    }
    ...
    _createClass(SectionBase, [{
      ...
      key: 'channels',
      get: function get() {
        return [].concat(_toConsumableArray(getGear(this).resolver.resolve()));
      }
    }]);
  ...
  var WhenBase = (function (_Common3) {
    ...
    function WhenBase(bus, parameters, targets) {
      ...
      var _this14 = _possibleConstructorReturn(this, Object.getPrototypeOf(WhenBase).call(this));

      setGear(_this14, new WhenGear(bus, parameters, targets));
      return _this14;
    }
