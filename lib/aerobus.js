...
(function (global, factory) {
  ...
  var CLASS_AEROBUS = 'Aerobus',
    ...
    throw new TypeError('Channel name expected to be a string, not "' + classof(value) + '".');
  },
    ...
    throw new TypeError('Trace expected to be a function, not "' + classof(value) + '".');
      ...
      throwUnexpectedParameter = function throwUnexpectedParameter(value) {
  ...
  var Subscriber = (function () {
    function Subscriber(next, done, name, order, retain) {
    ...
    _createClass(Subscriber, null, [{
      key: 'fromCallback',
      value: function fromCallback(next, name, order) {
        return new Subscriber(next, noop, name, order, true);
      }
    }, {
      key: 'fromObserver',
  ...
  var ChannelGear = (function () {
    ...
    _createClass(ChannelGear, [{
      ...
      key: 'forward',
      value: function forward(forwarders) {
        var _forwarders;

        this.trace('forward', forwarders);
        forwarders = forwarders.map(function (forwarder) {
          switch (classof(forwarder)) {
            case CLASS_FUNCTION:
            case CLASS_STRING:
              return forwarder;

            default:
              throwUnexpectedParameter(forwarder);
          }
        });
        if (forwarders.length) this.forwarders ? (_forwarders = this.forwarders).push.apply(_forwarders, _toConsumableArray(forwarders)) : this.forwarders = forwarders;
      }
    }, {
      key: 'publish',
      value: function publish(message, results) {
        var _this4 = this;
            ...
            (function () {
              ...
              forwarders.forEach(function (forwarder) {
                ...
                (isArray(names) ? names : [names]).forEach(function (name) {
                  if (isNothing(name) || false === name) skip = false;else if (isString(name)) destinations.add(name);else throwNameNotValid(name);
                });
              });
              ...
              try {
                for (var _iterator3 = destinations[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
                  ...
                  if (destination === _this4.name) skip = false;else getGear(_this4.bus.get(destination)).publish(message, results);
                }
              } catch (err) {
        ...
        subscribers.forEach(results ? function (subscriber) {
          try {
      ...
      value: function subscribe(parameters) {
        ...
        subscribers.forEach(function (subscriber) {
          var index = _this5.subscribers.findIndex(function (existing) {
            return existing.order > subscriber.order;
          });

          -1 === index ? _this5.subscribers.push(subscriber) : _this5.subscribers.splice(index, 0, subscriber);
          if (_this5.retentions && subscriber.retain !== false) _this5.retentions.forEach(function (message) {
            try {
              subscriber.next(message.data, message);
            } catch (error) {
              setImmediate(function () {
  ...
  var ChannelBase = (function () {
    ...
    _createClass(ChannelBase, [{
      ...
      value: function forward() {
        for (var _len2 = arguments.length, forwarders = Array(_len2), _key2 = 0; _key2 < _len2; _key2++) {
          forwarders[_key2] = arguments[_key2];
        }
      ...
      value: function publish(data, callback) {
        if (isSomething(callback)) {
          ...
          var results = [];
          getGear(this).publish(data, results);
          callback(results);
  ...
  var SectionBase = (function () {
    ...
    _createClass(SectionBase, [{
      ...
      key: 'forward',
      value: function forward() {
        for (var _len6 = arguments.length, forwarders = Array(_len6), _key6 = 0; _key6 < _len6; _key6++) {
          forwarders[_key6] = arguments[_key6];
        }
  ...
  function aerobus() {
    ...
    options.forEach(function (option) {
      switch (classof(option)) {
        ...
        default:
          throwUnexpectedParameter(option);
      }
    });
    ...
    function create() {
      ...
      overrides.forEach(function (override) {
        switch (classof(override)) {
          ...
          case CLASS_STRING:
            if (!override.length) throwDelimiterNotValid(override);
            overriden.delimiter = override;
            break;

          default:
            throwUnexpectedParameter(override);
        }
      });
