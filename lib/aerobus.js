...
(function (global, factory) {
  ...
  var CLASS_AEROBUS = 'Aerobus',
    ...
    return classof(value) === CLASS_PROMISE;
  },
    ...
    return classof(value) === CLASS_STRING;
      ...
      deliver = function deliver(bus, message, subscriber, transporter) {
    try {
      var result = subscriber.next(message.data, message);
      isPromise(result) ? result.next(transporter) : transporter(result);
    } catch (error) {
      transporter(error);
  ...
  var ChannelGear = (function () {
    ...
    _createClass(ChannelGear, [{
      ...
      key: 'forward',
      value: function forward(parameters) {
        this.trace('forward', parameters);
        var collection = this.forwarders || (this.forwarders = []),
            forwarding = new Forwarding(parameters);
        collection.push.apply(collection, _toConsumableArray(forwarding.forwarders));
      }
    }, {
      key: 'publish',
      value: function publish(message, transporter) {
        var _this4 = this;
            ...
            (function () {
              ...
              try {
                for (var _iterator3 = destinations[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
                  ...
                  if (destination === _this4.name) skip = false;else getGear(_this4.bus.get(destination)).publish(message, transporter);
                }
              } catch (err) {
        ...
        subscribers.forEach(function (subscriber) {
          return deliver(bus, message, subscriber, transporter);
        });
      }
    }, {
      key: 'reset',
      ...
      value: function subscribe(parameters) {
        ...
        builders.forEach(function (builder) {
          var subscriber = builder(),
              index = subscribers.findIndex(function (existing) {
            return existing.order > subscriber.order;
          });
          -1 === index ? subscribers.push(subscriber) : subscribers.splice(index, 0, subscriber);
          if (retentions && recall) retentions.forEach(function (message) {
            return deliver(bus, message, subscriber, noop);
          });
        });
      }
    }, {
      ...
      value: function publish(data, callback) {
        ...
        if (isSomething(callback)) {
          (function () {
            if (!isFunction(callback)) throw errorCallbackNotValid(callback);
            var results = [];
            getGear(_this10).apply('publish', data, function (result) {
              return results.push(result);
            });
            callback(results);
          })();
