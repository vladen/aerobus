...
(function (global, factory) {
	...
	describe('aerobus', function () {
		...
		it('root parent should return undefined', function () {
			...
			_chai.assert.ok((0, _aerobus.isUndefined)(bus.root.parent));
		});
		...
		it('error parent should return undefined', function () {
			...
			_chai.assert.ok((0, _aerobus.isUndefined)(bus.error.parent));
		});
		...
		it('extend should work with Channel', function () {
			var bus = _aerobus2.default.extend('Channel', {
				newMethod: function newMethod() {
					return 'test';
				}
			})(delimiter, trace);

			var channel = bus.root;

			_chai.assert.strictEqual(channel.newMethod(), 'test');
		});
		it('extend should work with Section', function () {
			var bus = _aerobus2.default.extend('Section', {
				newField: 'test'
			})(delimiter, trace);
		...
		it('extend should work independently', function () {
			var extendAerobus = _aerobus2.default.extend('Channel', {
				newField: 'test'
			});

			var bus1 = extendAerobus(delimiter, trace);
			var bus2 = (0, _aerobus2.default)(delimiter, trace);

			_chai.assert.strictEqual(bus1.root.newField, 'test');

			_chai.assert.strictEqual(bus2.root.newField, undefined);
		});
		it('extend should work chainly', function () {
			var aerobus1 = _aerobus2.default.extend('Channel', {
				newMethod: function newMethod() {
					return 'test';
				}
			});

			var bus = aerobus1.extend('Channel', {
				newField: 'test'
			})(delimiter, trace);
